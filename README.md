...
2. Then, we concatenate $t_{sou}$ with an instruction $t_{ins}$ and a learnable prompt $t_l$ — where $t_l$ is optimized via $\mathcal{L}_{\text{inv}}$ under the supervision of $R_{1...K}^{sou}$.
