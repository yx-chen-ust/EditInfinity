...
from infinity.models.basic import FFN
...
class Infinity(nn.Module):
    ...
    def autoregressive_infer_cfg(
        ...
        if infer_function == 0:
            ...
            replace_all_each_scale_code = True
        elif infer_function == 2:   
            save_ori_bg_each_scale_code = True

        assert sum([original_autoregressive_func, replace_all_each_scale_code, save_ori_bg_each_scale_code]) == 1,  "Only one of original_autoregressive_func, replace_all_each_scale_code, save_ori_bg_each_scale_code can be True"
        ...
        for si, pn in enumerate(scale_schedule):   # si: i-th segment
            ...
            for block_idx, b in enumerate(self.block_chunks):
                ...
                if self.add_lvl_embeding_only_first_block and block_idx == 0:
                    last_stage = self.add_lvl_embeding(last_stage, si, scale_schedule, need_to_pad)
                if not self.add_lvl_embeding_only_first_block: 
                    last_stage = self.add_lvl_embeding(last_stage, si, scale_schedule, need_to_pad)
                
                for module_idx,m in enumerate(b.module):
                    #print(f'module_idx: {module_idx}')
                    current_layer_idx = block_idx * len(b.module) + module_idx
                    last_stage = m(x=last_stage, cond_BD=cond_BD_or_gss, ca_kv=ca_kv, attn_bias_or_two_vector=None, attn_fn=attn_fn, scale_schedule=scale_schedule, rope2d_freqs_grid=self.rope2d_freqs_grid, scale_ind=si, word_token_mapping=word_token_mapping, target_words=target_words, layer_idx=current_layer_idx)
            ...
            if vae_type != 0:
                ...
                if si != num_stages_minus_1:
                    ...
                    if save_ori_bg_each_scale_code == True:
                        K_SCALES = [1, 2, 4, 6, 8, 12, 16, 20, 24, 32, 40, 48, 64]
                        #get background(dog+ball+bg) code
                        folder_path = os.path.join(infer_root_dir, infer_sub_dir, "each_scale_code/")
                        file_name = f"each_scale_code_{si}.pt"
                        file_path = os.path.join(folder_path, file_name)
                        loaded_tensor = torch.load(file_path)
                        ...
                        for x in range(64):
                            for y in range(64):
                                each_scale_code[:, :, :, x, y] = weighted_tensor[x, y] * loaded_tensor[:, :, :, x, y] + (1 - weighted_tensor[x, y]) * each_scale_code[:, :, :, x, y]
                    summed_codes += each_scale_code
            ...
            for b in self.block_chunks:
                for m in b.module:
                    if hasattr(m, 'ca') and hasattr(m.ca, 'final_attention_maps'):
                        if target_word in m.ca.final_attention_maps:
                            if final_attention_map is None:
    ...
    def replace_ffn_linear_with_lora(self, r=8, lora_alpha=16, lora_dropout=0.05):
        """Replace linear layers in FFN with LoRALinear layers."""
        ...
        for name, module in self.named_modules():
            if isinstance(module, FFN):
                ...
                if hasattr(module, 'fc1'):
                    ...
                    module.fc1 = LoRALinear(original_fc1, r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout)
                    # Ensure the original weights are properly transferred
                    module.fc1.original.weight.data = original_fc1.weight.data
                    if original_fc1.bias is not None:
                        module.fc1.original.bias.data = original_fc1.bias.data
            
                # Replace fc2
                if hasattr(module, 'fc2'):
                    original_fc2 = module.fc2
                    module.fc2 = LoRALinear(original_fc2, r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout)
                    # Ensure the original weights are properly transferred
                    module.fc2.original.weight.data = original_fc2.weight.data
                    if original_fc2.bias is not None:
                        module.fc2.original.bias.data = original_fc2.bias.data

    def replace_attention_linear_with_lora(self, r=8, lora_alpha=16, lora_dropout=0.05):
        from infinity.models.basic import LoRALinear
            ...
            if "CrossAttnBlock" in str(type(module)):
                ...
                if hasattr(module, "sa") and isinstance(module.sa, SelfAttention):
                    ...
                    if hasattr(sa, "proj") and isinstance(sa.proj, nn.Linear):
                        ...
                        sa.proj.original.weight.data = original_proj.weight.data
                ...
                if hasattr(module, "ca") and isinstance(module.ca, CrossAttention):
                    ...
                    if hasattr(ca, "proj") and isinstance(ca.proj, nn.Linear):
                        ...
                        ca.proj.original.weight.data = original_proj.weight.data
                        if original_proj.bias is not None:
                            ca.proj.original.bias.data = original_proj.bias.data   
