...
class AutoEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # Expose both integer indices (may be None for BSQ) and bit indices.
        # Keep 'encodings' for compatibility: map to bit_indices so downstream code expecting bit labels works.
        vq_output = {
            "commitment_loss": torch.mean(all_loss) * self.lfq_weight,
            "indices": all_indices,
            "bit_indices": all_bit_indices,
            "encodings": all_bit_indices,
        }
