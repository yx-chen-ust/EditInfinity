...
EX1_ROOT="/data1/chenyuxin/code/Infinity_clone/example_case/example_3"
PREPARE_SH="${EX1_ROOT}/scripts/prepare_edit.sh"
TRAIN_SH="${EX1_ROOT}/scripts/train_EditInfinity_example3.sh"
INFER_SH="${EX1_ROOT}/scripts/infer_EditInfinity_example3.sh"
...
echo "[Step 4] Get target word attention map"
ATTENTION_SH="${EX1_ROOT}/scripts/get_targetword_attentionmap_example3.sh"
update_var "$ATTENTION_SH" infer_function 1
# Ensure train roots are set consistently (derived again to be safe)
update_var "$ATTENTION_SH" infer_root_dir "\"$BASE_DIR\""
update_var "$ATTENTION_SH" infer_sub_dir "\"$SUB_DIR\""
bash "$ATTENTION_SH"

echo "[Step 5] Get weighted tensor and mask"
WEIGHTED_SH="${EX1_ROOT}/scripts/get_weighted_tensor.sh"
# Update paths in get_weighted_tensor.sh to use EX1_ROOT
update_arg "$WEIGHTED_SH" "--base_path" "$EX1_ROOT"
update_arg "$WEIGHTED_SH" "--case_path" "$EX1_ROOT"
bash "$WEIGHTED_SH"

echo "[Step 6] Final inference with background preservation"
...
update_var "$INFER_SH" infer_function 2
