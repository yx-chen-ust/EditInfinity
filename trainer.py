...
class InfinityTrainer(object):
    ...
    def train_step(
        ...
        with self.gpt_opt.amp_ctx:
            ...
            if self.reweight_loss_by_scale:
                lw = []
                last_scale_area = np.sqrt(torch.tensor(scale_schedule[-1]).prod())   #revice this code to avoid bug
                #print(f'last_scale_area: {last_scale_area}')
                for (pt, ph, pw) in scale_schedule[:training_scales]:
                    this_scale_area = np.sqrt(pt * ph * pw)
                    #print(f'this_scale_area: {this_scale_area}')
    ...
    def save_embedding(self, path):
        ...
        print(f"Mean: {np.mean(embedding_data):.4f}")
        print(f"Std: {np.std(embedding_data):.4f}")
        print(f"Min: {np.min(embedding_data):.4f}")
        print(f"Max: {np.max(embedding_data):.4f}")
        print("\nFirst 5x5 values:")
        print(embedding_data[:5, :5])
