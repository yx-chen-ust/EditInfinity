...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv, layer_idx=None, word_token_mapping=None, target_words=None):
        """
        :param q: shaped as (batch, seq_len, Q_dim)
        :param ca_kv: contains several vectors, each of which is shaped as (len_i, KV_dim). We have [len_1xKV_dim, len_2xKV_dim, len_3xKV_dim, ...] and lenses == [len_1, len_2, len_3, ...]
...
class CrossAttnBlock(nn.Module):
    ...
    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, attn_fn=None, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0, word_token_mapping=None, target_words=None, layer_idx=None):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.fused_norm_func is None:
            ...
            x = x + self.ca(self.ca_norm(x), ca_kv, layer_idx=layer_idx, word_token_mapping=word_token_mapping, target_words=target_words).float().mul_(self.ca_gamma)
            x = x + self.drop_path(self.ffn( self.ln_wo_grad(x.float()).mul(scale2.add(1)).add_(shift2) ).mul(gamma2)) # this mul(gamma2) cannot be in-placed cuz we possibly use FusedMLP
        else:
            #all run this branch 
